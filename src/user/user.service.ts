...
export class UserService {
  ...
  async profilePictureUpload(
    ...
    const resImageUpload = await this.imageUploadLib.imageUpload(
      imageUploadInput,
      this.PROFILE_PICTURE_UPLOAD_DIR,
      user,
    );
    let requestPictureUpload = new ImageUploadEntity();

    if (resImageUpload) {
      const userObj = await this._userRepository.getUserById(user.id);
      userObj.profile_picture = resImageUpload;
      userObj.updated_at = new Date(Date.now());
      this.userRepository.save(userObj);

      requestPictureUpload.success = true;
      requestPictureUpload.message = this.i18n.t(
        'requests.IMAGE_UPLOAD_SUCCESS',
      );
      requestPictureUpload.url = resImageUpload;
    } else {
      throw new BadRequestException(this.i18n.t('requests.IMAGE_UPLOAD_ERROR'));
    }
    return requestPictureUpload;
  }
  ...
  async restProfilePictureUpload(file: Express.Multer.File, user: User) {
    if (user.is_block) {
      throw new BadRequestException(this.i18n.t('user.USER_BLOCKED'));
    }
    const resImageUpload = await this.restImageUploadLib.imageUpload(
      file,
      this.PROFILE_PICTURE_UPLOAD_DIR,
      user,
    );
